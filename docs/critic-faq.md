...
The short answer is: probably not in the classic pool-hopping sense, but this
must be proven with open simulation.
...
That said, "probably not exploitable" is not the same as "proven." The right way
to settle this is to model explicit strategies:
...
a systematic expected-value advantage over honest continuous mining.
...
The current expectation is that departed miners are paid only for real work
already contributed, and their old proofs decay as new work arrives. The concern
is legitimate enough to deserve a dedicated pool-hopping simulation.
