...
- `gridlabs-science/datum-gateway-blake2b-gridpool`: GitHub fork of the community Blake2b DATUM implementation, now maintained at `innerhat-dev/datum_gateway`. The reviewed upstream base is `2fea7e5`; GridLabs `develop` is pinned at `6936c17`, including forced-coinbase compatibility, known-incompatible fail-closed handling, an explicitly logged unknown-firmware full-coinbase path, an appliance-safe UID-1000 runtime, and public multi-architecture publishing. The older `e894b8a` and `df502b3` pins are superseded.
