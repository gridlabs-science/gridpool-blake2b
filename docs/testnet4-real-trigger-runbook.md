...
Use this when moving a small GridPool test cluster from deterministic round resets to real GridPool block-triggered rounds on Bitcoin testnet4.

## Goal

Run at least two GridPool nodes, preferably three, on an isolated testnet4 GridPool network:
...
- GridPool state file: separate from mainnet, for example `pool_state.testnet4.json`
- Payout addresses: testnet addresses only (`tb1...`, testnet P2PKH, or testnet P2SH)

Do not reuse mainnet GridPool state or mainnet peer seeds for this test.
...
## Minimal GridPool Config

Each testnet4 GridPool node should use a config like:
...
sudo ./scripts/install-sovereign-stack.sh \
  --bitcoin-network testnet4 \
  --payout-address tb1... \
  --bootstrap-peers http://FIRST-TESTNET-GridPool-NODE:5000 \
  --yes
```
...
5. Set DATUM pool host/port to the local testnet4 GridPool node.
6. In GridPool config, set `bitcoin_network` to `testnet4`.
7. In GridPool config, set `boot_network_id` to `testnet4-beta`.
8. In GridPool config, set `pool_payout_script` to the same testnet address used by DATUM.
9. In GridPool config, set `testing_round_reset_mode` to `none`.
...
12. Start the first GridPool node with no bootstrap peers.
13. Start DATUM on the first node and confirm it gets fresh testnet4 templates.
14. Start second and third GridPool nodes with bootstrap peer pointed at the first node.
...
- Root landing page `https://gridpool.net` should not be used as a GridPool peer seed.
...
That means GridPool can reliably validate configured payout addresses against `bitcoin_network`, and it can decode slot-0 attribution in the configured network. But if a direct share submitter sends only raw coinbase/header data, GridPool cannot infer whether the miner originally thought of that script as `bc1...` or `tb1...`; it can only decode the script according to the local node's configured network.

For UI/API warnings, direct clients should submit their intended payout address metadata alongside the proof. GridPool can compare that metadata to slot-0 attribution and warn on mismatch, while still using slot-0 as the authoritative trustless attribution.
